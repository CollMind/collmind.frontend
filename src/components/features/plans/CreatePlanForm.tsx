...
import { channelEndpoints, categoryEndpoints, cplEndpoints, regionEndpoints, fuEndpoints } from '@/api/endpoints/master-data.endpoints';

interface CreatePlanFormProps {
  onSubmit: (data: CreatePlanDto & { selectedFuIds?: string[] }) => Promise<void>;
  onCancel?: () => void;
  isLoading?: boolean;
}
...
export function CreatePlanForm({ onSubmit, onCancel, isLoading = false }: CreatePlanFormProps) {
  ...
  const { data: fus } = useQuery({
    ...
    queryFn: () => fuEndpoints.getAll(true, undefined, formData.categoryId).then(res => res.data),
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            ...
            {errors.channelId && <p className="text-sm text-red-500 mt-1">{errors.channelId}</p>}
          </div>
          ...
          <div>
            ...
            {errors.cplId && <p className="text-sm text-red-500 mt-1">{errors.cplId}</p>}
          </div>
