...
export function PlanApprovalDetailModal({
  ...
  const { data: creatorUser } = useQuery({
    ...
    queryFn: () => userEndpoints.getById(planData.createdBy!).then(res => res.data),
    enabled: !!planData.createdBy,
  });

  // Calculate volumes
  const baseVolume = planData.planFus?.reduce(
      ...
      <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-3 gap-6 mt-4">
          ...
          <div className="col-span-1 space-y-4">
            ...
            <Card className="border-gray-200">
              <CardContent className="p-4 space-y-3">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="font-medium">{creatorUser?.fullName || planData.createdBy || 'N/A'}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="font-medium">
                    {planData.channel?.name || 'N/A'} • {planData.cpl?.name || 'N/A'}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <Layers className="h-4 w-4 text-gray-400" />
                  <span className="text-gray-600">Kategori:</span>
                  <span className="font-medium">{planData.category?.name || 'N/A'}</span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-gray-200">
              <CardHeader className="pb-3">
                <CardTitle className="text-sm font-semibold">KPI Özeti</CardTitle>
              </CardHeader>
              <CardContent className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-600">BASE VOL:</span>
                  <span className="font-medium">{formatNumber(baseVolume, 0)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">PLANNED:</span>
                  <span className="font-medium">{formatNumber(plannedVolume, 0)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">INCR.:</span>
                  <span className="font-medium">
                    +{formatNumber(incremental, 0)} ({formatPercentage(incrementalPercent)})
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatCurrency(incrementalGp)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">GP ROI:</span>
                  <span className={`font-medium ${isLowRoi ? 'text-red-600' : ''}`}>
          ...
          <div className="col-span-2 space-y-4">
            {/* FU Based Details */}
            <div>
              <h3 className="text-sm font-semibold text-gray-700 mb-3">FU Bazlı Detay</h3>
                      ...
                      <CardContent className="p-3">
                        ...
                        <div className="space-y-1 text-xs">
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                              {formatNumber(fuPlannedVolume, 0)} (+{formatPercentage(fuUplift)})
                            </span>
                          </div>
                          <div className="flex justify-between">
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <Badge
                              ...
                              className={isFuLowRoi ? 'bg-yellow-100 text-yellow-800' : ''}
                            >
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b border-gray-200">
                      <tr>
                        <th className="px-4 py-2 text-left text-xs font-semibold text-gray-600 uppercase">FU / SKU</th>
                        <th className="px-4 py-2 text-right text-xs font-semibold text-gray-600 uppercase">BASE VOL</th>
                        <th className="px-4 py-2 text-right text-xs font-semibold text-gray-600 uppercase">PLANNED VOL</th>
                        <th className="px-4 py-2 text-right text-xs font-semibold text-gray-600 uppercase">UPLİFT %</th>
                        <th className="px-4 py-2 text-right text-xs font-semibold text-gray-600 uppercase">FİYAT</th>
                        <th className="px-4 py-2 text-right text-xs font-semibold text-gray-600 uppercase">GP ROI</th>
                      </tr>
                    </thead>
                        ...
                        return (
                          <>
                            ...
                            <tr key={`fu-${planFu.id}`} className="bg-gray-50">
                              ...
                              <td className="px-4 py-2 text-right">{formatNumber(fuBaseVolume, 0)}</td>
                              <td className="px-4 py-2 text-right font-semibold">
                                {formatNumber(fuPlannedVolume, 0)}
                              </td>
                              <td className="px-4 py-2 text-right">{formatPercentage(fuUplift)}</td>
                              <td className="px-4 py-2 text-right">-</td>
                              <td className={`px-4 py-2 text-right ${fuRoi < 20 ? 'text-red-600' : ''}`}>
                                {formatPercentage(fuRoi)}
                              </td>
                            </tr>
                            {/* SKU Rows */}
                            {planFu.planSkus?.map((planSku) => {
                              ...
                              const skuPlannedVolume = planSku.plannedVolume || 0;
                              const skuIncremental = skuPlannedVolume - skuBaseVolume;
                              const skuUplift = skuBaseVolume > 0 ? ((skuIncremental / skuBaseVolume) * 100) : 0;
                              const skuPrice = planSku.sku?.unitPrice || 0;

                              return (
                                <tr key={`sku-${planSku.id}`} className="bg-white">
                                  <td className="px-4 py-2 pl-8 text-gray-700">
                                    {planSku.sku?.name || 'N/A'}
                                  </td>
                                  <td className="px-4 py-2 text-right">{formatNumber(skuBaseVolume, 0)}</td>
                                  <td className="px-4 py-2 text-right">{formatNumber(skuPlannedVolume, 0)}</td>
                                  <td className="px-4 py-2 text-right">{formatPercentage(skuUplift)}</td>
                                  <td className="px-4 py-2 text-right">{formatCurrency(skuPrice)}</td>
                                  <td className="px-4 py-2 text-right">-</td>
                                </tr>
                              );
                            })}
