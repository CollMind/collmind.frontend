...
export function PlanApprovalsPage() {
  ...
  const [budgetApprovalPlan, setBudgetApprovalPlan] = useState<Plan | null>(null);
  ...
  const approveMutation = useMutation({
    mutationFn: ({ planId, comments, autoCreateBudget, budgetAmount }: {
      planId: string;
      comments?: string;
      autoCreateBudget?: boolean;
      budgetAmount?: number;
    }) => planEndpoints.approve(planId, { comments, autoCreateBudget, budgetAmount }),
    onSuccess: () => {
    ...
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || 'Plan onaylanırken hata oluştu');
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || 'Plan reddedilirken hata oluştu');
    },
  });
  ...
  const avgRoi = plans && plans.length > 0
    ? plans.reduce((sum, p) => sum + (p.overallRoi || 0), 0) / plans.length
    : 0;
  const lowRoiCount = plans?.filter(p => (p.overallRoi || 0) < 20).length || 0;
      ...
      <div className="grid grid-cols-4 gap-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <div className="text-xs text-gray-500 uppercase">ORT. ROI</div>
                  <Badge variant="outline" className="text-xs bg-orange-50 text-orange-700 border-orange-200">
                    ORT.
                  </Badge>
                </div>
                <div className="text-2xl font-bold text-gray-900">{formatPercentage(avgRoi)}</div>
        ...
        <Card className="border-gray-200">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-xs text-gray-500 uppercase mb-1">ROI &lt; %20</div>
                <div className="text-2xl font-bold text-gray-900">{lowRoiCount}</div>
              </div>
        ...
        {filteredPlans.length === 0 ? (
          <Card>
            <CardContent className="p-8 text-center text-gray-500">
              {searchQuery ? 'Arama sonucu bulunamadı' : 'Onay bekleyen plan bulunmamaktadır'}
            </CardContent>
          </Card>
        ) : (
...
function PlanApprovalCard({
  ...
  const baseVolume = plan.planFus?.reduce(
    (sum, fu) => sum + (fu.planSkus?.reduce((s, sku) => s + (sku.baseVolume || 0), 0) || 0),
    0
  ) || 0;
  const plannedVolume = plan.totalPlannedVolume || 0;
  ...
  const skuCount = plan.planFus?.reduce((sum, fu) => sum + (fu.planSkus?.length || 0), 0) || 0;
    ...
    <Card className="border-gray-200">
      <CardContent className="p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-lg font-semibold text-gray-900">{plan.planCode}</span>
        ...
        <div className="flex items-center gap-4 text-sm text-gray-600 mb-4">
          ...
          <div className="flex items-center gap-1">
            ...
            <span>
              {plan.channel?.name || 'N/A'} • {plan.cpl?.name || 'N/A'} • {plan.category?.name || 'N/A'}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-4">
          <div>
            ...
            <div className="text-sm font-semibold text-gray-900">{formatCurrency(plan.totalSpend)}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500 mb-1">GP ROI</div>
            <div className="text-sm font-semibold text-gray-900">{formatPercentage(roi)}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500 mb-1">UPLIFT %</div>
            <div className="text-sm font-semibold text-gray-900">{formatPercentage(upliftPercent)}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500 mb-1">KAPSAM</div>
            <div className="text-sm font-semibold text-gray-900">{fuCount} FU / {skuCount} SKU</div>
          </div>
        </div>
        ...
        {isLowRoi && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4 flex items-start gap-2">
            ...
            <div className="text-sm text-yellow-800">
              ROI hedefin altında ({formatPercentage(roi)} &lt; %20). Taktiklerin gözden geçirilmesi önerilir.
            </div>
          </div>
        )}
