...
export function OffInvoiceUploadPage() {
  ...
  const handleFileUpload = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || 'Dosya yükleme başarısız oldu'
      );
    } finally {
  ...
  const handleImport = useCallback(async () => {
    ...
    setIsImporting(true);
    try {
      const rows: CreateOffInvoiceTransactionDto[] =
      ...
      toast.success(
        `${validationResult.validRows.length} satır başarıyla içe aktarıldı`
      );
      navigate('/off-invoice/transactions');
    } catch (error: any) {
      toast.error(
        error?.response?.data?.message || 'İçe aktarma başarısız oldu'
      );
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4 mb-8">
        <div
          className={`flex items-center ${step === 'upload' ? 'text-blue-600' : step === 'validation' || step === 'approval' ? 'text-green-600' : 'text-gray-400'}`}
        >
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${step === 'upload' ? 'border-blue-600 bg-blue-50' : step === 'validation' || step === 'approval' ? 'border-green-600 bg-green-50' : 'border-gray-300'}`}
          >
        ...
        <div
          className={`flex-1 h-0.5 ${step === 'validation' || step === 'approval' ? 'bg-green-600' : 'bg-gray-300'}`}
        />
        <div
          className={`flex items-center ${step === 'validation' ? 'text-blue-600' : step === 'approval' ? 'text-green-600' : 'text-gray-400'}`}
        >
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${step === 'validation' ? 'border-blue-600 bg-blue-50' : step === 'approval' ? 'border-green-600 bg-green-50' : 'border-gray-300'}`}
          >
            {step === 'approval' ? '✓' : '2'}
          </div>
        ...
        <div
          className={`flex-1 h-0.5 ${step === 'approval' ? 'bg-green-600' : 'bg-gray-300'}`}
        />
        <div
          className={`flex items-center ${step === 'approval' ? 'text-blue-600' : 'text-gray-400'}`}
        >
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${step === 'approval' ? 'border-blue-600 bg-blue-50' : 'border-gray-300'}`}
          >
            3
          </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="text-sm text-gray-600 mb-4">
            <p>
              ...
              <span className="font-medium">.xlsx, .csv</span>
            </p>
            <p>
          ...
          <div className="border-t pt-4 mt-6">
            ...
            <div className="mt-4 text-sm text-gray-600">
              <p className="mb-2">
                Yükleyeceğiniz dosyanın aşağıdaki kolonları içermesi
                gerekmektedir:
              </p>
              <ul className="list-disc list-inside space-y-1">
                <li>
                  <span className="font-medium">agreement_id</span> (ZORUNLU):
                  Anlaşma ID'si (örn: STA-2026-001)
                </li>
                <li>
                  <span className="font-medium">invoice_no</span> (ZORUNLU):
                  Fatura numarası
                </li>
                <li>
                  <span className="font-medium">invoice_date</span> (ZORUNLU):
                  YYYY-MM-DD formatında tarih
                </li>
                <li>
                  <span className="font-medium">fiscal_period</span> (ZORUNLU):
                  YYYY-MM formatında dönem (Bütçe buradan düşülür)
                </li>
                <li>
                  <span className="font-medium">amount</span> (ZORUNLU): İşlem
                  tutarı (Pozitif sayı)
                </li>
                <li>
                  <span className="font-medium">description</span> (OPSİYONEL):
                  İşlem açıklaması
                </li>
              </ul>
            </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-bold">
              {result.summary.affectedAgreements}
            </p>
          </div>
          <div>
            <p className="text-sm text-gray-600">UYARI</p>
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredRows.map((row) => (
                ...
                >
                  ...
                  <td
                    className={`px-6 py-4 whitespace-nowrap text-sm ${row.type === 'invalid' ? 'text-red-600 font-medium' : ''}`}
                  >
                    {row.agreementId}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                  ...
                  <td
                    className={`px-6 py-4 whitespace-nowrap text-sm ${row.type === 'invalid' && !row.fiscalPeriod ? 'text-red-600' : ''}`}
                  >
                    {row.fiscalPeriod || '-'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
