...
// bu dosyadaki referanslar da güncellendi — tel değeri ('FINANCE') hiç değişmedi.
describe('roleUtils', () => {
  describe('hasRole', () => {
    ...
    it('should return false when user does not have the required role', () => {
      ...
      expect(hasRole(UserRole.CATEGORY_MANAGER, [UserRole.FINANCE])).toBe(false);
  ...
  describe('hasAnyRole', () => {
    it('should return true for ADMIN regardless of allowed roles', () => {
      ...
      expect(hasAnyRole(UserRole.ADMIN, [UserRole.CATEGORY_MANAGER, UserRole.FINANCE])).toBe(true);
    });
    ...
    it('should return false when user does not have any of the allowed roles', () => {
      expect(hasAnyRole(UserRole.PLANNER, [UserRole.CATEGORY_MANAGER, UserRole.FINANCE])).toBe(false);
    });
  });
  ...
  describe('isRole', () => {
    ...
    it('should return false when user does not have the exact role', () => {
      ...
      expect(isRole(UserRole.CATEGORY_MANAGER, UserRole.FINANCE)).toBe(false);
    });
