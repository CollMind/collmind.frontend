...
export function OnInvoiceUploadPage() {
  ...
  const [validationResult, setValidationResult] =
    useState<ValidationResponseDto | null>(null);
  const [completionResult, setCompletionResult] =
    useState<CompletionResponseDto | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4 mb-8">
        <div
          className={`flex items-center ${step === 'upload' ? 'text-blue-600' : step === 'validation' || step === 'completed' ? 'text-green-600' : 'text-gray-400'}`}
        >
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${step === 'upload' ? 'border-blue-600 bg-blue-50' : step === 'validation' || step === 'completed' ? 'border-green-600 bg-green-50' : 'border-gray-300'}`}
          >
        ...
        <div
          className={`flex-1 h-0.5 ${step === 'validation' || step === 'completed' ? 'bg-green-600' : 'bg-gray-300'}`}
        />
        <div
          className={`flex items-center ${step === 'validation' ? 'text-blue-600' : step === 'completed' ? 'text-green-600' : 'text-gray-400'}`}
        >
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${step === 'validation' ? 'border-blue-600 bg-blue-50' : step === 'completed' ? 'border-green-600 bg-green-50' : 'border-gray-300'}`}
          >
            {step === 'completed' ? '✓' : '2'}
          </div>
        ...
        <div
          className={`flex-1 h-0.5 ${step === 'completed' ? 'bg-green-600' : 'bg-gray-300'}`}
        />
        <div
          className={`flex items-center ${step === 'completed' ? 'text-blue-600' : 'text-gray-400'}`}
        >
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${step === 'completed' ? 'border-blue-600 bg-blue-50' : 'border-gray-300'}`}
          >
            3
          </div>
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="flex items-start">
          ...
          <div>
            ...
            <p className="text-sm text-blue-800 mb-3">
              On-Invoice harcamalar, müşteri faturasında doğrudan uygulanan
              indirimlerdir. Bu veriler genellikle ERP veya satış sisteminden
              gelir ve bütçeden düşülmesi gerekir.
            </p>
            <div className="mb-3">
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded p-2 mt-3">
              <p className="text-xs text-yellow-800">
                <strong>▲ DİKKAT:</strong> Off-Invoice'dan farkı: On-Invoice
                müşterinin gördüğü faturada zaten düşülmüştür. Ayrı ödeme
                yapılmaz.
              </p>
            </div>
          </div>
      ...
      <div className="text-sm text-gray-600 mb-4">
        ...
        <p>
          Maksimum dosya boyutu: <span className="font-medium">10MB</span>
        </p>
      </div>
      ...
      <div className="border-t pt-4 mt-6">
        ...
        <div className="text-xs text-gray-600 space-y-1">
          <p>
            <code className="bg-gray-100 px-1 rounded">customer_code</code>: CPL
            ile eşleşen müşteri kodu
          </p>
          <p>
            <code className="bg-gray-100 px-1 rounded">invoice_no</code>: ERP
            fatura numarası
          </p>
          <p>
            <code className="bg-gray-100 px-1 rounded">fiscal_period</code>:
            Bütçe dönemi (YYYY-MM)
          </p>
          <p>
            <code className="bg-gray-100 px-1 rounded">discount_type</code>:
            İndirim tipi (CPP_ON, LTA_ON, PROMO_DISCOUNT)
          </p>
          <p>
            <code className="bg-gray-100 px-1 rounded">discount_amount</code>:
            Satır bazlı toplam indirim (₺)
          </p>
          <p>
            <code className="bg-gray-100 px-1 rounded">actual_price</code>:
            İndirimli birim fiyat
          </p>
          <p className="mt-2">
            * Ondalık ayracı olarak nokta (.) kullanılmalıdır.
          </p>
        </div>
      </div>
              ...
              <div>
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold">
                    {formatCurrency(
                      result.discountDistribution.cppOnInvoice.amount
                    )}
                  </span>
                </div>
                <div className="text-xs text-gray-500">
                  (
                  {result.discountDistribution.cppOnInvoice.percentage.toFixed(
                    0
                  )}
                  %)
                </div>
            ...
            {result.discountDistribution.ltaOnInvoice && (
              <div>
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold">
                    {formatCurrency(
                      result.discountDistribution.ltaOnInvoice.amount
                    )}
                  </span>
                </div>
                <div className="text-xs text-gray-500">
                  (
                  {result.discountDistribution.ltaOnInvoice.percentage.toFixed(
                    0
                  )}
                  %)
                </div>
            ...
            {result.discountDistribution.promoDiscount && (
              <div>
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold">
                    {formatCurrency(
                      result.discountDistribution.promoDiscount.amount
                    )}
                  </span>
                </div>
                <div className="text-xs text-gray-500">
                  (
                  {result.discountDistribution.promoDiscount.percentage.toFixed(
                    0
                  )}
                  %)
                </div>
